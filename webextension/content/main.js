...
async function uploadPGSG(data, screenshot) {

    data = JSON.stringify({ data: data, screenshot: screenshot})

    var response = await fetch(`${search_share_api}/submit`, {
      method: "POST",
    ...
    }).catch(err => console.log("Upload issue:", err))
...
async function prepareNotarizing(after_click) {
  ...
  if(active_tab.url.includes("duckduckgo.com")) {
    ddg = true;
    ddgUrl = "https://links.duckduckgo.com/d.js"
  } else {
    ddg = false;
  }
  ...
  var sessionId = await startNotarizing(rv.headers, rv.server, rv.port, oBSH_details.url)

  // Take screenshot to upload (with proof)
  var screenshot = await new Promise(function(resolve,reject) {
  ...
  var signedPage = await getPGSG(sessionId);

  // Send to API
  var sharePage = await uploadPGSG(signedPage, screenshot);

  console.log("share:", sharePage);
  // Open tab and load the shared search result
  setTimeout(() => {
    loadNormalIcon();
    chrome.tabs.create({ url: `${search_share_api}/share/${sharePage.result}`});
  }, 4000)
...
async function startNotarizing(headers, server, port, page_url) {
  ...
  try{
    ...
    let sessionId = await save_session(rv, page_url);
...
async function save_session(args, page_url) {  
  ...
  var commonName = page_url.substring(0,90);
...
function sendToManager(data, command) {
  ...
  if (is_chrome) {
    ...
    assert(['payload', 'export', 'upload'].includes(command))
    chrome.runtime.sendMessage({
      'destination': 'manager',
      'command': command,
      'payload': data
